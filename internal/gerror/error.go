...
// 标准 s3 错误集合，所有都以 awserr.RequestFailure 接口的形式返回

// GetError 根据 Error Code 获得具体的错误
func GetError(errcode APIErrorCode, origErr error) awserr.RequestFailure {

	e, ok := errorCodes[errcode]
	if ok {
		return awserr.NewRequestFailure(awserr.New(e.Code, e.Description, origErr), e.HTTPStatusCode, "")
	}

	e = errorCodes[ErrNotFoundError]

	return awserr.NewRequestFailure(awserr.New(e.Code, e.Description, origErr), e.HTTPStatusCode, "")
}

type errorMap map[APIErrorCode]APIError
...
var errorCodes = errorMap{
	...
	ErrNotFoundError: {
		...
		Description:    "Not Found S3 Error,please see OrgErr",
		HTTPStatusCode: http.StatusBadRequest,
	},
	ErrAccessKeyCreated: {
		Code:           "ErrAccessKeyCreated",
		Description:    "AccessKey Has Created",
		HTTPStatusCode: http.StatusBadRequest,
	},
}
